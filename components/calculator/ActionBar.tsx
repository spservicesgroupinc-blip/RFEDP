...
import { Save, CheckCircle2, Receipt, Calendar, Pencil, ClipboardList, HardHat, ArrowRight } from 'lucide-react';
...
export const ActionBar: React.FC<ActionBarProps> = ({
  ...
  return (
    <div className="md:col-span-2 flex flex-col gap-4 pt-6 border-t border-slate-200 pb-12">
      {/* Phase Status Label */}
      <div className={`inline-flex items-center px-3 py-1.5 rounded-lg font-semibold text-xs uppercase tracking-wide ${statusInfo.color} w-fit`}>
        {statusInfo.label}
      </div>
        ...
        {currentStatus === 'Paid' ? (
          ...
          <div className="flex-1 flex gap-2">
            {!activeScheduledDate ? (
              ...
              <button onClick={onStageWorkOrder} className="flex-none bg-white border-2 border-slate-100 hover:bg-slate-50 text-slate-500 p-4 rounded-xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2">
