...
export const Profile: React.FC<ProfileProps> = ({ state, onUpdateProfile, onManualSync, syncStatus, username, spreadsheetId, session }) => {
  const [isUploading, setIsUploading] = useState(false);
  const [isSavingCrew, setIsSavingCrew] = useState(false);
  const [crewSaveStatus, setCrewSaveStatus] = useState<'idle' | 'success' | 'error'>('idle');

  const handleCreateCrew = async () => {
    if (!session?.token || !state.companyProfile.crewUsername || !state.companyProfile.crewPassword) {
      alert('Enter crew email and password first.')
      return
    }
    setIsSavingCrew(true)
    setCrewSaveStatus('idle')
    try {
      const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/create-crew-user`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          crewEmail: state.companyProfile.crewUsername,
          crewPassword: state.companyProfile.crewPassword,
        }),
      })
      const json = await res.json()
      if (!res.ok) throw new Error(json.error || 'Failed to create crew account')
      setCrewSaveStatus('success')
    } catch (err: any) {
      console.error(err)
      setCrewSaveStatus('error')
      alert(err.message || 'Failed to create crew account.')
    } finally {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 focus-within:ring-2 focus-within:ring-brand focus-within:border-brand transition-all">
                        <label className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">
                            <User className="w-3 h-3"/> Crew Email
                        </label>
                        <input
                            type="email"
                            value={state.companyProfile.crewUsername || ''}
                            onChange={(e) => onUpdateProfile('crewUsername', e.target.value)}
                    ...
                    {/* Editable Crew Password */}
                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 focus-within:ring-2 focus-within:ring-brand focus-within:border-brand transition-all">
                        <label className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">
                            <KeyRound className="w-3 h-3"/> Crew Password
                        </label>
                        <input 
                            type="text" 
                            value={state.companyProfile.crewPassword || ''} 
                            onChange={(e) => onUpdateProfile('crewPassword', e.target.value)}
                            className="w-full bg-transparent font-mono font-bold text-xl text-slate-900 outline-none placeholder-slate-300"
                            placeholder="Create Crew Password"
                        />
                    </div>
                </div>
               ...
               {session?.role === 'admin' && (
                 <div className="mt-6 flex flex-col items-start gap-2">
                   <button
                     ...
                     className="bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 text-white font-black py-3 px-8 rounded-2xl flex items-center justify-center gap-2 uppercase text-xs tracking-widest transition-all active:scale-95 shadow-lg shadow-emerald-100"
                   >
                     {isSavingCrew ? <Loader2 className="w-4 h-4 animate-spin" /> : <Users className="w-4 h-4" />}
                     Create Crew Account
                   </button>
                   {crewSaveStatus === 'success' && (
                     <p className="text-xs font-bold text-emerald-600">Account Created!</p>
                   )}
                   {crewSaveStatus === 'error' && (
                     <p className="text-xs font-bold text-red-500">Failed — see alert</p>
                   )}
                 </div>
               )}

                <p className="mt-3 text-[10px] text-slate-400 font-medium italic">
                    * Updating the credentials requires a "Save & Sync" to take effect on crew devices.
                </p>
             </div>
